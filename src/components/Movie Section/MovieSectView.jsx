...
const MovieSectView = ({ title, movies, category }) => {
  return (
    <div>
      <div className="flex justify-between">
        <h1 className="text-xl font-bold text-zinc-800 dark:text-zinc-200">{title}</h1>
        <Link to={`/movie/${category}`} className="flex items-center bg-white dark:bg-zinc-800 py-1 px-3 rounded-xl text-xs gap-2">
